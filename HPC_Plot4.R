HPC_Plot4 <- function() {
  ...
  plot(HPC$timestamp, HPC$Global_active_power, type="l", xlab="Date", ylab="Global Active Power")
  
  #Here is the code for the second graph in our panel
  plot(HPC$timestamp, HPC$Voltage, type="l", xlab="Date",ylab="Voltage")
  ...
  legend("topright",col=c("black","blue", "red"), c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"), lty=c(1,1), 
         bty="n", cex=.5)
  plot(HPC$timestamp, HPC$Global_reactive_power, type="l", xlab="Date", ylab="Global_reactive_power")
  
  dev.copy(png,file="HPC_Plot4.png", width=800, height=800)
  dev.off()
